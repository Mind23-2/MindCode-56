...
测试集列表：原作者并未提供测试集列表，只能根据原作者给出的测试结果反推测试集列表。首先下载[blufr官方测试文件包][7]和[原作者测试结果][9]，将`blufr官方测试文件包`解压的文件夹，与`原作者测试结果--LightenedCNN_B_lfw.mat`、`LightCNN/src/get_list.py(本脚本提供)`放在同一个目录内，运行`python get_list.py`。

- 下载训练集和测试及测试集。

- 将下载的训练集(tsv文件)转为图片集。运行脚本: `bash scripts/convert.sh --file_name FILE_PATH --output_path OUTPUT_PATH`。
...
└──data
    ...
    │   │   ...
    │   │
    │   ├── image_list_for_blufr.txt                # lfw BLUFR protocols 测试图片列表
    │   └── image_list_for_lfw.txt                  # lfw 6,000 pairs 测试图片列表
    │
    └── MS-Celeb-1M_clean_list.txt                  # 清洗后的训练图片列表
```
...
- 生成config json文件用于8卡训练。
     ...
     # 10.111.*.*为计算机IP地址, 也可以将IP地址填写为127.0.0.1
     ```
...
│   ├── dataset.py                                  # 加载训练数据集
│   ├── get_list.py                                 # 获取测试集列表
│   ├── get_multicards_json.py                      # 获取8卡Ascend配置信息
